...
class TestGraphTransferClient:
    ...
    @pytest.mark.transfer
    def test_upload_file_to_sharepoint_large_file(self, transfer_client, mock_upload_session_response, sample_file_info):
        """大きなファイルのアップロードテスト"""
        # 検証対象: upload_file_to_sharepoint()
    ...
    def test_create_upload_session_error(self, transfer_client):
        ...
        with patch('requests.post') as mock_post:
            mock_post.return_value.raise_for_status.side_effect = requests.exceptions.HTTPError("Session creation failed")
