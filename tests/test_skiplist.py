...
from src.skiplist import load_skip_list, is_skipped, add_to_skip_list, save_skip_list
...
class TestSkipList:
    ...
    def setup_method(self):
        """各テストメソッドの前処理"""
        self.test_skip_list = [
            {"path": "/test/file1.txt", "name": "file1.txt", "reason": "already_exists"},
            {"path": "/test/file2.txt", "name": "file2.txt", "reason": "permission_error"}
        ]
    ...
    def test_load_skip_list_existing_file(self):
        ...
        with patch('os.path.exists', return_value=True):
            with patch('builtins.open', 
                      mock_open(read_data=json.dumps(self.test_skip_list))):
