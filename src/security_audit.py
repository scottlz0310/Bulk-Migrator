...
class SecurityAuditor:
    ...
    def __init__(self, audit_log_path: str = "logs/security_audit.log"):
        ...
        self.sensitive_files = {".env", "config.json", "*.key", "*.pem", "*.p12"}
    ...
    def audit_file_access(self, file_path: str, operation: str, user: str = None):
        """ファイルアクセスの監査ログ"""
        ...
        if any(pattern in file_path.lower() for pattern in patterns):
            self.audit_logger.warning(
                f"SENSITIVE_FILE_ACCESS - User: {user}, Operation: {operation}, "
                f"File: {self._mask_path(file_path)}, PID: {os.getpid()}"
            )
        else:
            self.audit_logger.info(
                f"FILE_ACCESS - User: {user}, Operation: {operation}, "
                f"File: {self._mask_path(file_path)}"
            )

    def audit_api_call(self, endpoint: str, method: str, status_code: int = None):
        """API呼び出しの監査"""
        self.audit_logger.info(
            f"API_CALL - Endpoint: {self._mask_endpoint(endpoint)}, "
            f"Method: {method}, Status: {status_code or 'N/A'}"
        )

    def audit_authentication(
        self, success: bool, user: str = None, details: str = None
    ):
        """認証イベントの監査"""
        user = user or os.getenv("USERNAME", "unknown")
        ...
        self.audit_logger.warning(
            f"AUTH_{status} - User: {user}, Details: {details or 'N/A'}, "
            f"Timestamp: {datetime.utcnow().isoformat()}Z"
        )

    def check_file_integrity(self, critical_files: list[str]) -> dict[str, Any]:
        """重要ファイルの整合性チェック"""
        integrity_report = {
        ...
        for file_path in critical_files:
            if os.path.exists(file_path):
                ...
                integrity_report["files"][file_path] = {
                    "hash": file_hash,
                    "size": len(content),
                ...
                if os.path.exists(hash_file):
                    ...
                    if stored_hash != file_hash:
                        ...
                        integrity_report["alerts"].append(alert)
    ...
    def scan_for_secrets_exposure(self, directory: str = ".") -> dict[str, Any]:
        """機密情報の露出スキャン"""
        exposed_secrets = []
...
class AccessController:
    ...
    def check_file_permissions(self, file_path: str) -> dict[str, Any]:
        ...
        # 機密ファイルの権限チェック
        if any(
            sensitive in file_path.lower() for sensitive in [".env", "secret", "key"]
        ):
        ...
        return {"status": "SECURE", "path": file_path, "permissions": permissions}
    ...
    def secure_file_permissions(self, file_path: str) -> bool:
        ...
        try:
            if any(
                ...
                for sensitive in [".env", "secret", "key"]
            ):
