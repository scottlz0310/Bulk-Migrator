...
try:
    ...
    LOG_PATH = os.environ.get(
        "TRANSFER_LOG_PATH", "logs/transfer_start_success_error.log"
    )
...
class SecureLogger:
    ...
    SENSITIVE_PATTERNS = [
        ...
        # JSON内の機密情報
        (
            r'"(client_secret|access_token|refresh_token|api_key|password|secret|token)"\s*:\s*"([^"]+)"',
            r'"\1": "[MASKED]"',
        ),
    ]
    ...
    def _setup_logger(self, log_path: str | None = None):
        ...
        file_handler = RotatingFileHandler(
            actual_log_path, maxBytes=5 * 1024 * 1024, backupCount=3, encoding="utf-8"
        )
    ...
    def log_auth_event(self, event: str, details: dict[str, Any] | None = None):
        """認証イベントログ（セキュア版）"""
        ...
        if details:
            ...
            for key, value in details.items():
                if any(
                    ...
                    for sensitive in ["secret", "token", "password", "key", "tenant"]
                ):
