...
from src.skiplist import is_skipped, load_skip_list
...
class GraphTransferClient:
    def upload_file_to_sharepoint(
        ...
        src_root: str = "TEST-Onedrive",
        dst_root: str = "TEST-Sharepoint",
        timeout: int = 10,
    ) -> dict[str, Any]:
        """
        ...
        if file_size >= threshold_bytes:
            logger.info(
                f"大容量ファイル検出({file_size} bytes) - アップロードセッション使用",
                file_size=file_size,
    ...
    def filter_skipped_targets(
        self, file_targets: list[dict[str, Any]], skip_list_path: str | None = None
    ) -> list[dict[str, Any]]:
        """
    ...
    def collect_file_targets_from_onedrive(
        self,
        folder_path: str,
        ...
        """
        logger = get_structured_logger("transfer")
        logger.info("OneDriveクロール開始", folder_path=folder_path)
    ...
    def list_onedrive_items_with_path(
        self,
        user_principal_name: str | None = None,
        drive_id: str | None = None,
        folder_path: str = "",
        ...
        try:
            ...
            logger.warning(
                "API応答がありません（タイムアウト）",
                url=url,
                method="list_onedrive_items_with_path",
            )
            return []
        except requests.exceptions.RequestException as e:
            logger = get_structured_logger("transfer")
        ...
        for part in path_parts:
            ...
            try:
                ...
                if resp.status_code == 404:
                    # フォルダが存在しないので作成
                    logger = get_structured_logger("transfer")
                    logger.info("フォルダを作成", folder_path=current_path)
