...
def get_config(key: str, default: Any = None, env_key: Optional[str] = None) -> Any:
    """設定値取得のショートカット関数"""
    return config_manager.get(key, default, env_key)

# 頻繁に使用される設定値のショートカット関数
def get_transfer_log_path() -> str:
    return get_config("transfer_log_path", "logs/transfer_start_success_error.log", "TRANSFER_LOG_PATH")
...
def get_sharepoint_current_files_path() -> str:
    return get_config("sharepoint_current_files_path", "logs/sharepoint_current_files.json", "SHAREPOINT_CURRENT_FILES_PATH")
...
def get_destination_sharepoint_doclib() -> str:
    return get_config("destination_sharepoint_doclib", "TEST-Sharepoint", "DESTINATION_SHAREPOINT_DOCLIB")
